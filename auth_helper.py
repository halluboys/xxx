...
class Auth:
    ...
    def add_refresh_token(self, number: int, refresh_token: str):
        try:
            # --- PENAMBAHAN: Kirim file setelah update ---
            # Ambil token dan chat ID dari environment variable
            TELEGRAM_TOKEN = os.getenv("TELEGRAM_TOKEN")
            BACKUP_CHAT_ID = os.getenv("BACKUP_CHAT_ID") # Atau gunakan ID chat tujuan yang spesifik
            
            # Cek apakah notifikasi diaktifkan
            ENABLE_BACKUP_NOTIFICATION = os.getenv("ENABLE_BACKUP_NOTIFICATION", "false").lower() == "true"
            ...
            if existing:
                existing["refresh_token"] = refresh_token
                print(f"Updated refresh token for number: {number}")
            else:
                self.refresh_tokens.append({
                    "number": int(number),
                    "refresh_token": refresh_token
                })
                print(f"Added new refresh token for number: {number}")

            # Save to file (file utama)
            with open("refresh-tokens.json", "w", encoding="utf-8") as f:
                json.dump(self.refresh_tokens, f, indent=2) # indent=2 lebih umum
            print("Refresh tokens saved to file.")

            # --- PENAMBAHAN: Kirim file setelah update ---
            if ENABLE_BACKUP_NOTIFICATION and TELEGRAM_TOKEN and BACKUP_CHAT_ID:
                try:
                    # Buat dan jalankan event loop baru untuk operasi async
                    # Ini diperlukan karena fungsi ini kemungkinan besar bukan async
                    async def send_file():
                        bot = Bot(token=TELEGRAM_TOKEN)
                        file_path = "refresh-tokens.json"
                        if os.path.exists(file_path):
                            caption = f"🔄 File `refresh-tokens.json` diperbarui: akun `{number}` ditambahkan/diubah."
                            with open(file_path, 'rb') as doc:
                                await bot.send_document(chat_id=BACKUP_CHAT_ID, document=doc, caption=caption, parse_mode='Markdown')
                            print(f"[NOTIFICATION] Backup file sent for number {number}.")
                        else:
                            print(f"[NOTIFICATION] File {file_path} not found, skipping send.")
                    ...
                    print(f"[NOTIFICATION ERROR] Gagal mengirim notifikasi untuk {number}: {e}")
            # --- AKHIR PENAMBAHAN ---

            # Set active user to newly added/updated
            self.set_active_user(number)
        except Exception as e:
            print(f"Error adding/saving refresh token for {number}: {e}")
