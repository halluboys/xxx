...
async def _process_family_qris_payment(
    query: Update.callback_query, context: ContextTypes.DEFAULT_TYPE,
    ...
    await query.message.edit_text("🔄 Memproses pembayaran QRIS untuk paket Family...")
    ...
    try:
        ...
        if not payment_methods_
            await query.message.edit_text("❌ Gagal mendapatkan metode pembayaran QRIS untuk paket Family.")
            return
        ...
        if not qris_data:
            await query.message.edit_text("❌ Gagal mendapatkan data QRIS untuk paket Family.")
            return
...
# --- HANDLER UNTUK INPUT AMOUNT MANUAL ---
# Karena kita meminta input teks (amount), kita perlu handler untuk itu.
# Handler ini akan dipicu oleh MessageHandler untuk pesan teks biasa ketika state adalah 'waiting_family_qris_amount'.
# Handler ini sudah ada di bottytel.txt: application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_family_code_input))
# Tapi handle_family_code_input sudah digunakan untuk Family Code. Kita butuh handler baru atau memodifikasi handle_family_code_input.
# Untuk kesederhanaan, kita akan MODIFIKASI handle_family_code_input untuk menangani state baru ini juga.

# --- MODIFIKASI handle_family_code_input ---
# Temukan fungsi handle_family_code_input yang sudah ada dan MODIFIKASI bagian awalnya seperti ini:
# (Anda menemukannya di sekitar baris 370-an di bottytel.txt)

# Ganti fungsi handle_family_code_input yang ada dengan versi berikut:
async def handle_family_code_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == 'waiting_family_qris_amount':
        ...
        # Karena ini dari MessageHandler, kita butuh objek query palsu atau cara lain.
        # Cara termudah: Buat Update.callback_query yang mirip.
        dummy_query = update
        dummy_query.message = update.message
        ...
        # State sudah dihapus di dalam _process_family_qris_payment
        return # Selesai
...
# ... (fungsi-fungsi lain seperti buy_family_with_pulsa, buy_family_with_ewallet)
# === INFORMASI AKUN ===
