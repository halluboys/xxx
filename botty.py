    ...
    if current_state == 'waiting_quick_login_number':
        ...
        try:
            ...
            if user_exists:
                ...
                if success:
                    ...
                    await update.message.reply_text(
                        ...
                        f"Silakan login ulang untuk nomor ini melalui menu 'Login/Ganti akun'."
                    )
                    # Reset state
                    if 'state' in context.user:
                        del context.user_data['state']
                ...
                await update.message.reply_text(
                    ...
                    f"Silakan lanjutkan dengan proses login biasa."
                )
                # Arahkan ke proses login biasa
                context.user_data['temp_phone'] = phone_number
                context.user_data['state'] = 'waiting_phone_number' # Ubah state ke login biasa
            ...
            await update.message.reply_text(
                ...
                "Silakan coba lagi atau gunakan menu 'Login/Ganti akun'."
            )
            # Reset state
            if 'state' in context.user:
                del context.user_data['state']
