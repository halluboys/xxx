...
async def handle_phone_number_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
   ...
   if current_state == 'waiting_quick_login_number':
   	await process_quick_login(update, context, phone_number)
...
# === HANDLER CALLBACK ===
# Di bagian handler bot_telegram.py, tambahkan:

async def process_quick_login(update: Update, context: ContextTypes.DEFAULT_TYPE, phone_number: str) -> None:
    """Memproses login cepat dengan nomor yang dikirim"""
    await update.message.reply_text("🔄 Memeriksa status nomor...")
    
    try:
        # 1. Muat ulang token untuk mendapatkan daftar akun terkini
        AuthInstance.load_tokens()
        users = AuthInstance.refresh_tokens
        
        # 2. Periksa apakah nomor sudah ada dalam daftar
...
async def quick_login_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Simpan state bahwa user sedang dalam proses login cepat
