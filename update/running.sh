...
if [[ $tls_v2ray_status == "running" ]]; then 
   xray.service=" ${GREEN}Running${NC} ( No Error )"
else
   xray.service="${RED}  Not Running${NC}   ( Error )"
fi

# STATUS SERVICE NON TLS V2RAY
if [[ $nontls_v2ray_status == "running" ]]; then 
   xray.service=" ${GREEN}Running ${NC}( No Error )${NC}"
else
   xray.service="${RED}  Not Running ${NC}  ( Error )${NC}"
fi
...
if [[ $strgo == "active" ]]; then
  ...
  trojan-go.service="${RED}  Not Running ${NC}  ( Error )${NC}"
fi
