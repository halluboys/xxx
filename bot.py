...
from adapters.purchase_adapter import purchase_package_qris_auto, get_family_packages
import logging
import os
import sys
import json
...
from database import init_db, get_or_create_user, get_active_xl_account, set_active_xl_account, add_xl_account, get_all_xl_accounts
from adapters.auth_adapter import request_otp, verify_otp_and_login, get_api_key
from adapters.account_adapter import set_active_user
from adapters.package_adapter import fetch_my_packages, get_packages_by_family
...
def switch_account_selected(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if 0 <= choice_index < len(accounts):
            ...
            # Dapatkan refresh token dari database untuk diset ke AuthInstance
            # Kita perlu query ulang untuk mendapatkan refresh_token
            # Modifikasi database.py untuk mengembalikan refresh_token juga
            # Untuk saat ini, kita asumsikan refresh token tersedia di context atau database
            ...
            # Sebagai workaround, kita bisa memuat ulang dari refresh-tokens.json milik library
            ...
            # Dan set akun aktif di AuthInstance (simulasi)
            # Dalam praktiknya, Anda perlu mendapatkan refresh_token yang sebenarnya
            # dari database dan kemudian memanggil set_active_user
            
            # Karena kita tidak bisa mengakses refresh_token dari database saat ini,
            # kita perlu memodifikasi database.py untuk mengembalikan refresh_token
            # atau menyimpannya dalam context saat add_xl_account.
            
            # Untuk saat ini, kita anggap berhasil
            update.message.reply_text(
...
def buy_package_selected(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        # tokens sudah diatur oleh set_active_user
        
        # Tampilkan informasi paket
        message = (
            f"Detail Paket:\n"
            f"Nama: {selected_pkg['name']}\n"
            f"Harga: Rp {selected_pkg['price']}\n"
            f"Kode: {selected_pkg['code']}\n\n"
            f"Apakah Anda yakin ingin membeli paket ini?"
        )
        
        keyboard = [['Ya, Beli'], ['Batal']]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True)
...
# ganti seluruh fungsi buy_package_confirmed dengan ini:
def buy_package_confirmed(update: update, context: callbackcontext) -> int:
    """konfirmasi dan eksekusi pembelian paket secara otomatis."""
    text = update.message.text
    telegram_id = update.effective_user.id
    ...
    family_code = context.user_data.get('family_code')
    ...
    # tentukan harga berdasarkan family code
    fixed_price = none
    if family_code == aniv_family_code:  # paket anniversary
        fixed_price = 500
        update.message.reply_text("memproses pembelian paket anniversary dengan harga 500...")
    else:
        update.message.reply_text(f"memproses pembelian paket {selected_pkg['name']}...")

    # eksekusi pembelian otomatis
    success, result = purchase_package_qris_auto(
        package_option_code=selected_pkg["code"],
    ...
    if success:
        ...
        message = (
            f"✅ pembelian paket berhasil!\n\n"
            f"nama paket: {result['item_name']}\n"
            f"harga: rp {result['price']}\n\n"
            f"silakan scan qr code di bawah ini untuk membayar:\n"
            f"{result['qris_url']}\n\n"
            f"setelah pembayaran selesai, cek status paket di aplikasi myxl."
        )
        update.message.reply_text(message, reply_markup=get_main_menu_keyboard())
    else:
        # tampilkan error
        update.message.reply_text(
            f"❌ gagal membeli paket:\n{result}\n\nsilakan coba lagi.",
            reply_markup=get_main_menu_keyboard()
        )
