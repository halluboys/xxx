...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    main_buttons = [
	InlineKeyboardButton("1. Login & Cek Login", callback_data='switch_account_menu'),
...
async def buy_aniv_package_direct(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    - Package Number: 7
    - Harga API = 0, tapi harus dipaksa 500 saat QRIS
    """
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        AMOUNT_ANIV = 500  # <--- FIX: harga tetap Rp500
        ...
        all_packages_list, option_number = [], 1
        for variant in package_variants:
            for option in variant["package_options"]:
        ...
        if not target_package_info:
            await query.message.edit_text(f"❌ Paket nomor {PACKAGE_NUMBER_ANIV} ({PACKAGE_NAME_ANIV}) tidak ditemukan.")
            return
        ...
        # Simpan data sementara untuk diproses QRIS
        ...
        await query.message.edit_text("❌ Terjadi kesalahan saat memulai pembelian paket Aniv.")
