...
# 2. Tambahkan decorator ke handler yang ingin dilindungi
@authorized_only
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    # ... KODE MENU UTAMA ANDA DI SINI ...
    # (Tidak perlu diubah, hanya tambahkan @authorized_only di atasnya)
    
@authorized_only
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await show_main_menu(update, context)

# 3. Modifikasi button_handler (karena tidak bisa pakai decorator)
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    user_id = query.from_user.id

    # === TAMBAHKAN PENGECEKAN OTORISASI DI SINI ===
    if not is_user_authorized(user_id):
    ...
    await query.answer() # Hapus query.answer() lain di bawah jika ada
    # ... SISA KODE BUTTON_HANDLER ANDA DI SINI ...


# 4. Tambahkan handler baru untuk admin
