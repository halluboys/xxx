...
def settlement_qris(
    ...
    price: int,
    item_name: str = "",
    force_amount: bool = False  # tambahan parameter
):
    """
    Settlement QRIS.
    - price: nilai yang caller beri (biasanya price dari API).
    - force_amount=True -> pakai price sebagai final_price (misal: 500) di semua field harga.
    """

    # final_price: nilai yang akan digunakan di semua field harga
    final_price = price if not force_amount else price

    # Jika di masa depan kamu mau default otomatis 500 untuk force case:
    # final_price = price if not force_amount else 500
    ...
    # Debug: print payload (hapus atau comment saat production)
    try:
        print("DEBUG settlement_payload (pre-encrypt):")
    ...
    sig_time_sec = (xtime // 1000)
    x_requested_at = datetime.fromtimestamp(sig_time_sec, tz=timezone.utc).astimezone()
    # gunakan ts_to_sign sebagai timestamp final sesuai alur sebelumnya
    settlement_payload["timestamp"] = ts_to_sign
    ...
    # debug response raw (hapus/comment jika sensitif)
    try:
        print("DEBUG raw response status:", resp.status_code)
    ...
    try:
        ...
        if decrypted_body["status"] != "SUCCESS":
            print("Failed to initiate settlement.")
            print(f"Error: {json.dumps(decrypted_body, indent=2)}")
            return None
...
# Di akhir fungsi show_qris_payment, ubah return statement menjadi:
# return qris_code, qris_url

# Atau buat fungsi baru yang mengembalikan data QRIS:
def get_qris_data(api_key: str, tokens: dict, package_option_code: str, token_confirmation: str, price: int, item_name: str = ""):
    """Get QRIS data without displaying it"""
    print("Fetching payment method details...")
    payment_methods_data = get_payment_methods(
        api_key=api_key,
        tokens=tokens,
